...
static void uavoMavlinkRxBridgeTask(void *parameters);
...
//static void ProcessBuf(uint8_t *buf);
...
//static uint8_t *rx_stream_ticks;
static mavlink_message_t *mav_rx_msg;
...
static int32_t uavoMavlinkBridgeStart(void)
{
    if (module_enabled) {
		...
		xTaskCreate(uavoMavlinkRxBridgeTask, "uavoMavlinkRxBridge", STACK_SIZE_BYTES / 4, NULL, RX_TASK_PRIORITY, &rxtaskHandle);
		PIOS_TASK_MONITOR_RegisterTask(TASKINFO_RUNNING_UAVOMAVLINKRXBRIDGE, rxtaskHandle);
		return 0;
    }
    return -1;
}
...
static int32_t uavoMavlinkBridgeInitialize(void)
{
    if (PIOS_COM_MAVLINK) {
		...
		mav_rx_msg = pios_malloc(sizeof(*mav_rx_msg));

        if (mav_msg && stream_ticks && mav_rx_msg ) {
...
static void uavoMavlinkBridgeTask(__attribute__((unused)) void *parameters)
{
    ...
    while (1) {
        ...
        for (unsigned i = 0; i < MAXSTREAMS; ++i) {
			...
			if(i != MAV_DATA_STREAM_EXTRA1)continue;
